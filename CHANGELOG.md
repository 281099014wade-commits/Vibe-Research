...
### 发布前审计（Codex 多轮评审，20 条确认修复）

发布前对相对上游基线的全部改动跑了多轮独立代码评审，确认并修复 20 条
（含 3 条 P1）：阶梯概率单调化（报价噪声会让分布总和超 1）、Prometheus 非有限样本
过滤（NaN 进缓存会炸 JSON 序列化）、Kalshi 分页与 cursor 转义、刷新加锁防并发丢更新、
AI 工具输出裁剪进 chat 上限并保留新鲜度元数据、报价三态判定（缺席≠非法数值）、
日 K 窗口收紧防旧价冒充当前、单边结算月不画成精确点、图表容器 ResizeObserver、
文档依赖清单对齐等。另有两条上游缺陷已在各自仓库发补丁版
