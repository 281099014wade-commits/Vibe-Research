...
- **近一年走势**：B200 / H100 SXM / A100 SXM4 逐日中位价折线（约 365 点/条），
  ...
  口径已与现货互验：`quantile(0.5, …{rented="no"})` 的当前值与 Vast bundles 全市场
  中位完全一致。
...
- 新增 31 项离线测试（解析 / 三分判别 / 概率推导 / 失败回填 / API 契约）。
...
- **`a-stock-data/` 同步至上游 v3.7.0**（十一层 54 端点 19 数据源：新增筹码分布 CYQ /
  估值历史 / 复权因子等，详见其 CHANGELOG）。
- **资讯雷达同步上游 investment-news v1.0.2**：① 源清单清掉 2 个跨栏重复源（108 → 106）；
...
> ③ Vast 查询必须用**精确型号名**（`H100 SXM` 等）——写 `H100` 会永远查到 0 条报价，
> 看起来像「没人在租」，其实是查错了名字。
