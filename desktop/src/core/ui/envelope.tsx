...
export function Metric({
  ...
  return (
    <div className="min-w-0">
      ...
      {/* 常驻显示**资料期**(它是读数的一部分:同一个数字换个期就是另一回事);
          证据 id 收进 tooltip —— 一屏十几个 `ev-8a250ba36c` 是噪音,而**溯源不需要它常驻**,
          要的时候悬停就有。⚠️ 收起来 ≠ 去掉:这条链是这个产品的立身之本,不能断。 */}
      <div
        className="mt-0.5 truncate text-[10.5px] text-muted-foreground"
        title={ev ? `资料期 ${ev.period} · 证据 ${ev.id}` : "无此项"}
      >
        {ev ? ev.period : "无此项"}
      </div>
    </div>
