...
import { DueBadge, LedgerShell, enumLabel } from "../components/ui/RecordForm";
...
export function Risk() {
  ...
  return (
    <div className="space-y-4">
      ...
      <LedgerShell>{(s) => <Body s={s} openDock={openDock} />}</LedgerShell>
    </div>
  );
}

function Body({ s, openDock }: { s: LedgerView; openDock: (seed?: string) => void }) {
  ...
  return (
    <>
      ...
      <button
        ...
        onClick={() =>
          openDock(
            "看我的集中度与证伪条件:这套持仓在什么情形下会同时受伤(共同风险因子是什么)?只讲因子,不讲仓位该怎么调。",
          )
        }
