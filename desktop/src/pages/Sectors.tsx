...
import { MoneyCell, PctCell } from "../components/ui/envelope";
import { Async, Badge, Card, CardHead, cx } from "../components/ui/primitives";
import { api } from "../lib/api";
import { labelOf, num, pivot, sortBy, type Row } from "../lib/records";
...
export function Sectors() {
  ...
  const fn = useCallback(
    () =>
      Promise.all([
        api.fetch({ endpoint: "em_industry_comparison" }),
        // 🔴 默认 top_n=50 且按净额排,而涨跌排名有 100 个 ⇒ 只有 15 个对得上,整张表全是"未覆盖"。
        //    实测要到 200 才基本覆盖住,耗时几乎不变(1.3s → 1.8s)。上游共 496 个板块。
        api.fetch({ endpoint: "em_board_fund_flow", args: { top_n: 200 } }),
        api.fetch({ endpoint: "sw_industry", symbol: "300308" }),
      ]),
    [],
  );
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardHead
          title="这一页在回答什么"
          note="今天哪些行业在涨、钱往哪儿去,两件事对不对得上。对得上=有资金跟随;只涨不进钱=情绪。"
        />
      </Card>
          ...
          return (
            <>
              ...
              <button
                ...
                onClick={() =>
                  openDock(
                    "就今天的行业板块表,找出「涨得多但主力净额是流出」的板块,说明这种背离通常意味着什么、要确认还需要哪些数据。",
                  )
                }
