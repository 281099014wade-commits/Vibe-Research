...
import { api, type LedgerRecord } from "../lib/api";
import { num, pivot } from "../lib/records";
import { useUi } from "../lib/store";
import { useAsync } from "../lib/useAsync";
import { dueState, numOf, recordsOf, str, type LedgerView } from "../lib/useLedger";
...
export function Operate() {
  ...
  return (
    <div className="space-y-4">
      ...
      <LedgerShell>{(s) => <Body s={s} form={form} setForm={setForm} openDock={openDock} />}</LedgerShell>
    </div>
  );
}
...
function Body({
  ...
  const fetchQuotes = useCallback(
    () => (codes.length === 0 ? Promise.resolve(null) : api.fetch({ endpoint: "tx_quotes_batch", args: { codes } })),
  ...
  return (
    <>
      ...
      <button
        type="button"
        onClick={() =>
          openDock("对照我的持有与我写下的判据:哪几笔的理由已经站不住了?逐条说明是哪条判据被什么事实推翻的。")
        }
