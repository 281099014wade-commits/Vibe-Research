...
- **就在本仓库的 [`a-stock-data/`](a-stock-data/) 文件夹里**（v3.7.0）。十一层数据架构、54 个端点、19 个数据源，`a-stock-data/SKILL.md` **内嵌全部调用代码**，自包含、零第三方数据封装依赖，东财接口已内置限流防封，主源被封还能降级到备用源。
...
```
Vibe-Research/
├── a-stock-data/      A 股全栈数据工具箱（数据源，v3.7.0，自带即用）
