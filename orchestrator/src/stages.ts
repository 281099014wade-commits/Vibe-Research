...
const EXT_GUIDE: Record<Stage, string> = {
  ...
  以上每一类若有值得记录的事实,写 extra_findings(topic ∈ {"资金行为","解禁","股东结构","公告线索","互动易","新闻线索","其他线索"} 之一,summary 只报数 ≤ 600 字,evidence_ids 必填且**必须同时列在本阶段顶层 evidence_ids**);解禁 / 大宗 / 两融变化适合写进 decision_points 的"下一个数据点"。`,
  report: `- 扩展章节(可选,放在「风险与反证」之后、「裁决点」之前):## 资金与市场行为(两融 / 大宗 / 龙虎榜 / 解禁 / 股东户数 / 资金流 / 筹码,只报数 + ev id)/ ## 公告 · 互动易 · 新闻线索(标题级线索 + ev id);各阶段 extra_findings 汇总进对应章节;没有就不写。必需章节集不变。`,
};
